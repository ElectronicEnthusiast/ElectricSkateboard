...
//	  NOT USED: DAC-C  ---> Resolver carrier excitation
...
Uint32 IsrTicker = 0; //Keeps track of times Control Loop Interrupt is Fired
Uint32 SVGenMacroCount_LEVEL1 = 0; //
...
// ******************************************************************************
	...
	// Position Sense Reversal
	//posEncElecTheta[RESOLVER_POS_ENCODER] = 1.0 - resolver1.ElecThe;
...
// ****************************************************************************
	...
	InitEPwm4Gpio();
	InitEPwm5Gpio();
	InitEPwm11Gpio(); //added in case PWM11 is not triggering interrupt for SVPWM
						//NOTE: PWM11 controls the entire SVPWM control algorithm
...
//TODO BUILD 1
#if (BUILDLEVEL == LEVEL1) // Module check out (Open Loop Motor PWM Control)
...
// ------------------------------------------------------------------------------
	...
	// Verifying this LEVEL1 portion of the ISR is working
 	SVGenMacroCount_LEVEL1++;
...
//	This is used for graphing the space vector generation on CCS IDE graphing features
// ------------------------------------------------------------------------------
	DlogCh1 = rg1.Out;
	DlogCh2 = svgen1.Ta;
	DlogCh3 = svgen1.Tb;
...
//------------------------------------------------------------------------------
	if (rslvrIn.TUNING)
	{
		...
		DacaRegs.DACVALS.bit.DACVALS = rslvrOut.angleOut*4096;
	}
	else
	{
...
//TODO INCRBUILD 2
#if (BUILDLEVEL==LEVEL2) // Verify ADC, park/clarke, calibrate the offset and speed measurement
...
//	EPwm1Regs.TZSEL.bit.OSHT1         = 1;           // 1/0 - Enable/Disable One Shot Mode
...
//	EPwm3Regs.TZCTL.bit.TZB = TZ_FORCE_LO; // EPWMxB will go low
	...
	EPwm1Regs.TZCTL.bit.TZA = TZ_NO_CHANGE; // EPWMxA will not trip
	EPwm1Regs.TZCTL.bit.TZB = TZ_NO_CHANGE; // EPWMxB will not trip
	EPwm2Regs.TZCTL.bit.TZA = TZ_NO_CHANGE; // EPWMxA will go low
	EPwm2Regs.TZCTL.bit.TZB = TZ_NO_CHANGE; // EPWMxB will go low
	EPwm3Regs.TZCTL.bit.TZA = TZ_NO_CHANGE; // EPWMxA will go low
	EPwm3Regs.TZCTL.bit.TZB = TZ_NO_CHANGE; // EPWMxB will go low
